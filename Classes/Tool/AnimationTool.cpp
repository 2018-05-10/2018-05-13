...
Animation* AnimationTool::ExplosionDieAnimation(int x, int y)
{
	Vector<SpriteFrame*> frameVec;
	SpriteFrame* frame;
	frame = GameScene::_frameCacheOfExplosion->getSpriteFrameByName(StringUtils::format("Explosion_attack_(%d,%d)_3.png", x, y));
	frameVec.pushBack(frame);
...
Animation* AnimationTool::ExplosionMoveAnimation(int x, int y)
{
	...
	for (int i = 1; i < 4; i++)
	{
		frame = GameScene::_frameCacheOfExplosion->getSpriteFrameByName(StringUtils::format("Explosion_attack_(%d,%d)_%d.png", x, y, i));
		frameVec.pushBack(frame);
	}
	frame = GameScene::_frameCacheOfExplosion->getSpriteFrameByName(StringUtils::format("Explosion_move_(%d,%d).png", x, y));
	frameVec.pushBack(frame);
